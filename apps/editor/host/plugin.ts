...
import { createMaterial, deleteAssetFile, deleteMap, deleteMaterial, deleteModel, deleteTexture, deleteTextureMap, importAsset, loadMap, renameMaterial, saveMap, saveMaterial, saveModelMeta, scanAssets, scanMaps } from "./files";
    ...
    configureServer(server) {
      server.middlewares.use((req, res, next) => {
        ...
        if (method === "POST" && url === "/__editor/texture") {
          readBody(req).then((body) => {
            const b = JSON.parse(body);
            const r = b.op === "clearMap" ? deleteTextureMap(root, b.name, b.slot) : deleteTexture(root, b.name);
            json(res, (r as { error?: string }).error ? 400 : 200, r);
          }).catch((e) => json(res, 500, { error: String(e) }));
          return;
        }
